...
// A skeleton for a ext-angular workspace.json file.
const workspaceJson = {
    apps: [],
...
printUsage = () => {
    console.log(
`
    ext-angular CLI currently offers a method for generating a theme for ext-angular apps (SASS based system).

    An example usage is:
        ext-angular generate theme --baseTheme theme-material --name my-awesome-theme
...
/**
 * Ensures a 'ext-angular/packages' folder exists for the workspace and theme packages to be installed in.
 */
const ensurePackagesFolder = () => {
    return new Promise(resolve => {
        const dir = path.join('.', 'ext-angular', 'packages');
    ...
    return ensurePackagesFolder().then(() => {
        return new Promise((resolve, reject) => {
            if(!workspaceExists()) {
                ...
                fs.writeFile(path.join('.', 'ext-angular', 'workspace.json'), JSON.stringify(workspaceJson, null, 4), err => {
                    if(err) return reject(err);
                    return resolve();
                });
            } else {
                console.log('Using existing workspace at ext-angular/workspace.json');
                return resolve();
            }
        });
    });
...
const workspaceExists = () => {
    try {
        return fs.statSync(path.join('.', 'ext-angular', 'workspace.json')).isFile();
    } catch(e) {
        return false;
    }
}
...
const generateTheme = config => {
    ...
    return new Promise((resolve, reject) => {
        const proc = fork(sencha, [
            ...
            '--framework', 'ext',
            '--name', config.name
        ], { cwd: path.join('.', 'ext-angular'), silent: true });
...
const updatePackageJson = config => {
    const packageJsonPath = path.join('.', 'ext-angular', 'packages', config.name, 'package.json');
...
const applyTheme = config => {
  ...
  return new Promise((resolve, reject) => {
      fs.writeFile('.ext-angularsrc', JSON.stringify({theme: path.join('.', 'ext-angular', 'packages', config.name)}, null, 4),
      err => {
          if(err) {
            return reject(err);
          }
...
switch(args._.join(' ')) {
    ...
    case 'generate theme': {
        ...
        return generateWorkspace(args)
            ...
            .then(() => {
              console.log(`Theme created at: ext-angular/packages/${args.name}`);
